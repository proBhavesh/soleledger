...
import { plaidClient, PLAID_PRODUCTS, PLAID_COUNTRY_CODES, PLAID_REDIRECT_URI } from "@/lib/plaid/client";
...
export async function createLinkToken() {
    ...
    try {
        ...
        if (error.isAxiosError) {
            ...
            errorDetails.requestId = response.headers?.['x-request-id'] || response.headers?.['request-id'];
            errorDetails.plaidError = response.data?.error_code || response.data?.error_message;
        ...
        throw new Error(`Failed to create Plaid link token: ${errorDetails.message}`);
    }
}
...
export async function exchangePublicToken(
    ...
    try {
        ...
        for (const account of accountsResponse.data.accounts) {
            ...
            const metadataAccount = metadata.accounts.find(a => a.id === account.account_id);
 ...
 */
async function syncTransactions(accessToken: string, businessId: string, userId: string) {
    try {
        const now = new Date();
