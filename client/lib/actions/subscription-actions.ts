...
import { createStripeCustomer, createStripeCheckoutSession } from "@/lib/stripe/server";
...
export async function startTrialAction(planType: PlanType) {
    ...
    try {
        ...
        return {
            success: true,
            message: `Your trial has started! It will end on ${format(trialEndsAt, 'PPP')}`,
...
export async function requestEnterpriseCallbackAction(formData: FormData) {
    ...
    try {
        ...
        console.log("Enterprise request:", { name, email, company, phoneNumber, message });
