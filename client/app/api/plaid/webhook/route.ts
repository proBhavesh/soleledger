...
export async function POST(req: NextRequest) {
    try {
        ...
        switch (webhook_type as WebhookType) {
            case "TRANSACTIONS":
                await handleTransactionsWebhook(webhook_code as WebhookCode, item_id, payload);
                break;
        ...
        switch (webhookCode) {
            ...
            case "TRANSACTIONS_REMOVED": {
                ...
                if (removed_transactions && Array.isArray(removed_transactions)) {
                    await removeTransactions(bankAccount.businessId, removed_transactions);
                }
                break;
            }
 ...
 */
async function syncTransactions(bankAccount: any) {
    try {
        const now = new Date();
 ...
 */
async function removeTransactions(businessId: string, transactionIds: string[]) {
    try {
